...
```{r start building the data model,echo=TRUE, include=TRUE}
rpdr_conn <- DBI::dbConnect(RSQLite::SQLite(), dbname="rpdr.sqlite")

rpdr_db_map <- dm::dm_from_src(rpdr_conn)
...
```{r initial attempt at data model viz}
rpdr_db_map %>% 
  dm_add_pk(cntstnt, season) %>% 
  dm_set_colors(green=c(cntstnt)) %>% 
  dm_draw()
