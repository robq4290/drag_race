...
```{sql, connection="drag_conn"}
-- Found one missing value in season 13 going to manually change it in the dev db 
select
    rtng.air_date
  , rtng.viewer_rating
  , rtng.viewer_count 
from vw_ep_rtng rtng
inner join vw_episode_season_ratings esr

on rtng.air_date=esr.air_date

where esr.viewer_count is null 
...
```{r Initialize esquisse to get a look at rating data}
