...
Going to Normalize the data so that it will be easier to run multiple tests
across different measures... that way I won't have to go in column by column!
...
Looks like the raw data violated the normality assumption 

Going to use the log transformations in the ANOVA , it should account for 
...
Time to run the ANOVA on these. Also going to include the post hoc

Tukey Honest Significant Differences 



```{r Filter the input to the transformed variables and run ANOVA}
# using the %like% operator from data.table
anova_model<- views_ratings_model_df %>% 
  ...
  mutate(anova_model=map( data
         ...
         , tukey_test=map(anova_model,stats::TukeyHSD)
         , tukey_results=map(tukey_test, broom::tidy)
...
Check out the Tukey HSD results 
